...
public class BookStore {
    ...
    static ArrayList<Book.Item> sShoppingCart = new ArrayList<>();
                ...
                if (sBooks[index].hasEbook) {
                    ...
                    while (true) {
                        ...
                        if (useEbookOption.equalsIgnoreCase("yes")) {
                            ...
                            Book.Item item = sBooks[index].toItem(true);
                            sShoppingCart.add(item);
        ...
        if (sShoppingCart.size() == 0) {
            ...
            while (true) {
                ...
                if (option == 0) {
                    System.out.println();
                    return;
                } else if (option <= sShoppingCart.size()) {
                    int index = option - 1;
                    // when deleted item is physical, plus 1 to physical book num
                    if (!sShoppingCart.get(index).isEbook) {
                        sShoppingCart.get(index).book.copies++;
                    }
                    sShoppingCart.remove(index);
    ...
    private static void checkout() {
        if (sShoppingCart.size() == 0) {
            ...
            printShoppingCart();
            double sum = 0;
            for (int i = 0; i < sShoppingCart.size(); i++) {
                sum += sShoppingCart.get(i).price;
            }
            sShoppingCart.clear();
    ...
    private static void printShoppingCart() {
        ...
        for (int i = 0; i < sShoppingCart.size(); i++) {
            System.out.printf("%d. ", (i + 1));
            sShoppingCart.get(i).showItemInfo();
        }
    }
}